	...
	<head>
		...
		<script type="text/javascript">
			...
			$(document).ready(function(){
				...
				$("#genome_advanced").find("option[value='"+value+"']").attr("selected",true);
				
				showTab('geneAdvanced');
				if($("#searchForm").val().trim()!=''){
					$("#output").attr("style","display: _block");
					$("#se_img").attr("style","margin-top: 5px; display: _block");
					$("#se_etype").css('display','none');
					$("#se_source").css('display','none');
					$("#isExpand").val(0);
				}
			...
			function hiddenNames() {
				...
				if(isExpand == 1)
				{
					...
					$("#se_etype").attr("style","margin-top: 5px; display: _block");
					$("#se_source").attr("style","margin-top: 5px; display: _block");
			...
			<div id="output" class="row" style="display: none;">
				...
				<div id="relateinfo" class="span10 text-center">
					<c:if test="${!isHistoryResult}">
						<div class="row" id="topinfo">
							...
							<c:if test="${totalRecords gt 0}">(${result.usedTime } <fmt:message key="label.seconds"/>) <fmt:message key="label.about"/> 
								 ...
								 (hg19): Chr: ${result.current.seqNameShort }, Start: ${result.current.start }, End: ${result.current.end }, Strand: ${result.current.strand }
							</c:if>
						</div>
					...
					<div class="row paginateWrap">
						...
						<c:if test="${page gt 1}"><a href="javascript:;" class="paginatebtn" onclick="prevPage_advanced();"><fmt:message key="button.previous"/></a></c:if>
						<c:if test="${totalPage == 0 }">
							<a href="javascript:;" class="inactive">1</a> 
						</c:if>
						<c:forEach var="aPage" begin="${beginPage}"  end="${endPage}">
							<span href="javascript:;" <c:if test="${page eq aPage}">class="current paginate"</c:if> <c:if test="${page != aPage}">class="paginate advanced"</c:if>
						...
						<c:if test="${page lt totalPage}"><a href="javascript:;" class="paginatebtn" onclick="nextPage_advanced();"><fmt:message key="button.next"/></a></c:if>
					</div>
							...
							<c:forEach items="${result.sampleItemList}" var="item" varStatus="status">
								...
								<!--  <div class="sampleRow"   id="moreBtnDiv_${item.sampleId }" style="position: absolute;  display: none; text-align: left; margin-left: 4px;">
									<ul class="inline">
										...
										<li><a class="btn btn-mini" id="showTop5Gene${item.sampleId }" href="javascript:;" onclick="showTop5Genes(${item.sampleId },true)" title="" targetDrag="${item.sampleId }_Drag" style="display: block;">Top 10 Gene</a></li>
									</ul>
								</div>-->
