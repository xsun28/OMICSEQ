...
<!DOCTYPE html>
	...
	<head>
		...
		<script type="text/javascript">
			...
			$(document).ready(function(){
				rootPath = '${ctx}';
				showTab('datasetSearch');
				if($("#searchForm_datasetSearch_cell").val().trim()!=''|| $("#searchForm_datasetSearch_factor").val().trim()!=''){
				...
				if (!!!isHistoryResult || isHistoryResult == "false") {
					...
					//显示统计图
					var charFileName = $("#charFileName").val();
					var totalRecords = "${totalRecords}";
					if(charFileName != "" && totalRecords > 0)
					{
						$("#dialogbutton3").attr("style","display: _block; margin-left: 12px;");
						var diagramDrag = document.getElementById("diagramDrag");
						initDragDialog(diagramDrag,3);
						var imgSrc = rootPath + "/servlet/DisplayChart?filename="+ charFileName;
			...
			$(function(){
				var user = '${user.name}';
				if(user == 'emorybiostat'){
					$('td[id^=cell_]').dblclick(function (){
						var cellTDId = $(this).attr("id");
						var cellVal = $(this).text();
						var sampleId = cellTDId.split("_")[1];
						$(this).html('<input id="cellNew_'+ sampleId +'" value="'+ cellVal +'" onblur="saveCellChange(' + sampleId + ');">');
					});
					
					$('td[id^=factor_]').dblclick(function (){
						var factorTDId = $(this).attr("id");
						var factorVal = $(this).text();
						var sampleId = factorTDId.split("_")[1];
						$(this).html('<input id="factorNew_'+ sampleId +'" value="'+ factorVal +'" onblur="saveDetailChange(' + sampleId + ');">');
					});
					
					$('td[id^=lab_]').dblclick(function (){
						var labTDId = $(this).attr("id");
						var labVal = $(this).text();
						var sampleId = labTDId.split("_")[1];
						$(this).html('<input id="labNew_'+ sampleId +'" value="'+ labVal +'" onblur="saveLabChange(' + sampleId + ');">');
					});
				}
			});
			
			function saveCellChange(sampleId){
				rootPath = '${ctx}';
				var cellNew = $("#cellNew_"+sampleId).val();
				var cellOld = $("#cellOld_"+sampleId).val();
				if(cellOld != cellNew && cellNew != ''){
					var text = 'Are you sure change Cell from "' + cellOld + '" to " '+cellNew +'"';
					if(confirm(text)){
						$.ajax({
							url : rootPath + "/modifyCell.json",
							data : {
								cellOld : cellOld,
								cellNew : cellNew
							},
			...
			function saveLabChange(sampleId){
				rootPath = '${ctx}';
				var labNew = $("#labNew_"+sampleId).val();
				var labOld = $("#labOld_"+sampleId).val();
				if(labOld != labNew && labNew != ''){
					var text = 'Are you sure change Lab from "' + labOld + '" to " '+labNew +'"';
					if(confirm(text)){
						$.ajax({
							url : rootPath + "/modifyLab.json",
							data : {
								labOld : labOld,
								labNew : labNew
							},
			...
			function saveDetailChange(sampleId){
				rootPath = '${ctx}';
				var cell = $("#cell_"+sampleId).text();
				var factorNew = $("#factorNew_"+sampleId).val();
				var factorOld = $("#factorOld_"+sampleId).val();
				if(factorOld != factorNew && factorNew != ''){
					var text = 'Are you sure change Detail from "' + factorOld + '" to " '+factorNew +'"';
					if(confirm(text)){
						$.ajax({
							url : rootPath + "/modifyDetail.json",
							data : {
								factorNew : factorNew,
								factorOld : factorOld,
								cell : cell,
						...
						$("#factor_"+sampleId).html(factorOld);
					}
				}else{
					$("#factor_"+sampleId).html(factorOld);
				}
			...
			<div id="output" class="row" style="display: none;">
				<div id="relateinfo" class="span10 text-center">
					<c:if test="${!isHistoryResult}">
						<div class="row" id="topinfo">
							<fmt:message key="button.search"/> "<strong>
								<c:if test="${cell !=''}"><fmt:message key="label.cell"></fmt:message>:${cell }</c:if>
								<c:if test="${cell != '' && factor != '' }">,</c:if>
								<c:if test="${factor !=''}"><fmt:message key="label.factor"></fmt:message>:${factor }</c:if>
					...
					<div class="row paginateWrap">
						...
						<c:if test="${page gt 1}"><a href="javascript:;" class="paginatebtn" onclick="prevPage_datasetSearch();"><fmt:message key="button.previous"/></a></c:if>
						<c:if test="${totalPage == 0 }">
							<a href="javascript:;" class="inactive">1</a> 
						</c:if>
						<c:forEach var="aPage" begin="${beginPage}"  end="${endPage}">
							<span href="javascript:;" <c:if test="${page eq aPage}">class="current paginate"</c:if> <c:if test="${page != aPage}">class="paginate datasetSearch"</c:if>
						...
						<c:if test="${page lt totalPage}"><a href="javascript:;" class="paginatebtn" onclick="nextPage_datasetSearch();"><fmt:message key="button.next"/></a></c:if>
					</div>
					<div class="row">
						<input type="hidden" id="sortType" value="${sortType}"/>
						<input type="hidden" id="charFileName" value="${charFileName}">
						<table id="result_detail" class="table table-striped table-bordered table-hover table-condensed">
							<tr>
								...
								<th width="10%"><fmt:message key="label.factor"/></th>
								...
								<th width="23%"><fmt:message key="label.moreinfo"/></th>
							</tr>
							...
							<c:forEach items="${result.sampleItemList}" var="item" varStatus="status">
								<tr class="sampleRow">
									...
									<td class="cell" title="${item.cell_desc }" nowrap="nowrap" id="cell_${item.sampleId }">${item.cell }</td>
									<input id="cellOld_${item.sampleId }" type="hidden" value="${item.cell }">
									<td class="factor" title="${item.factor_desc}" nowrap="nowrap" id="factor_${item.sampleId }">${item.detail}</td>
									<input id="factorOld_${item.sampleId }" type="hidden" value="${item.detail }">
									<td class="study" title="${item.study }" nowrap="nowrap"
										style="background:<c:if test="${item.study == 'TCGA'}">#fdc7c7</c:if>
											...
											"	
									>${item.study }</td>
									<td id="lab_${item.sampleId }" class="lab" title="${item.lab}">${item.lab}</td>
									<input id="labOld_${item.sampleId }" type="hidden" value="${item.lab }">
									...
									<td align="left" id="Btntd_${item.sampleId }">
										<ul class="inline">
											...
											<!--  
											<li>
												<select name="top5Genes" id="top5Genes${item.sampleId }" class="btn btn-mini" onchange="javascript:searchGene(${item.sampleId});">
    												<option selected="selected" >the five largest readings</option>
    												<c:forEach items="${item.top5Genes }" var="gene">
    													<option>${gene }</option>
    												</c:forEach>
												</select>
											</li>
								...
								<div class="sampleRow"   id="moreBtnDiv_${item.sampleId }" style="position: absolute;  display: none; text-align: left; margin-left: 4px;">
									<ul class="inline">
	...
	<c:if test="${!isHistoryResult}">
		<a class="open btn btn-primary dialogbutton3" id="dialogbutton3" href="javascript:;" onclick="showDialog(this);" title="Diagram" targetDrag="diagramDrag" style="display:none; margin-left: 12px;">D</a>
