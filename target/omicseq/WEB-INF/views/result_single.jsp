...
<!DOCTYPE html>
    ...
    <div id="wrap">
      ...
      <div class="container" style="width: 90%;">
        	...
        	<!-- 搜索页面 -->
				<%@ include file="/WEB-INF/views/search_single.jsp"%>
			</div>
			<div id="output" class="row" style="display: none;">
				...
				<div id="relateinfo" class="span10 text-center">
					<c:if test="${!isHistoryResult}">
						<div class="row" id="topinfo">
							...
							<c:if test="${totalRecords gt 0}">(${result.usedTime } <fmt:message key="label.seconds"/>) <fmt:message key="label.about"/> 
								 ...
								 <br>(hg19): Chr: ${result.current.seqNameShort }, Start: ${result.current.start }, End: ${result.current.end }, Strand: ${result.current.strand }
							</c:if>
						</div>
							...
							<div class="row linkRow">
								...
								<c:forEach items="${geneItemList}" var="geneitem" varStatus="status" begin="0" end="10">
									<c:if test="${geneitem.txName != result.current.txName }">
										<span class="linkWord_single">${geneitem.txName }<c:if test="${status.count lt geneSize}">,</c:if></span>
									</c:if>
								</c:forEach> 
					...
					<div class="row paginateWrap">
						...
						<c:if test="${page gt 1}"><a href="javascript:;" class="paginatebtn" onclick="prevPage_single();"><fmt:message key="button.previous"/></a></c:if>
						<c:if test="${totalPage == 0 }">
							<a href="javascript:;" class="inactive">1</a> 
						</c:if>
						<c:forEach var="aPage" begin="${beginPage}"  end="${endPage}">
							<span href="javascript:;" <c:if test="${page eq aPage}">class="current paginate_single"</c:if> <c:if test="${page != aPage}">class="paginate_single"</c:if>
						...
						<c:if test="${page lt totalPage}"><a href="javascript:;" class="paginatebtn" onclick="nextPage_single();"><fmt:message key="button.next"/></a></c:if>
					</div>
    ...
    <%@ include file="../common/footer_single.jsp"%>
