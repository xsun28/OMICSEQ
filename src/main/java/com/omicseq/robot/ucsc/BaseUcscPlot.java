...
public abstract class BaseUcscPlot extends AbstractLifeCycle {
    ...
    protected List<GeneRankCriteria> buildCriterias(boolean all) {
        ...
        if (all) {
            // 添加组合查询 条件 ,暂未实现
            Object[] objs = new Object[] {SourceType.ENCODE, SourceType.Roadmap,  ExperimentType.CHIP_SEQ_TF};
            int len = objs.length;
            int bit = 1 << len;
            for (int i = 1; i <= bit; i++) {
    ...
    static abstract class BaseCallable<T, R extends BaseUcscPlot> implements
			Callable<T> {
		protected R ref;
