...
public class Copy_2_of_CNVParser {
	...
	public List<String> readMethylationFile(){
		...
		if(mefile.isDirectory()){
			...
			for(String me : meFileNames){
				if(me.startsWith("ACC")){
					meFileList.add(me);
				}
			}
//			for(int i=0; i<meFileList.size(); i++){
//				if(meFileList.get(i).contains(".rda" )|| meFileList.get(i).startsWith("GBM") ||meFileList.get(i).startsWith("LAML")){
//					meFileList.remove(i);
//					i--;
//				}
//			}
	...
	public void parser (){
		...
		for(String meFileName : meFileList){
			...
			File file = new File(root+"GeneLevel"+File.separator+meFileName);
			...
			try {
				...
				System.out.println("start="+startSampleId+" end="+endSampleId);
