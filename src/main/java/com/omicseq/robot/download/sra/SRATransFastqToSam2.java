...
public class SRATransFastqToSam2 implements Runnable {
    ...
    public SRATransFastqToSam2(FileInfo fileInfo) {
    ...
    public SRATransFastqToSam2(String name) {
        this.name = name;
    }
...
//        new SRATransFastqToSam(info).run();
        ...
        SRATransFastqToSam2 fts = new SRATransFastqToSam2();
        Thread processThread = new Thread(fts);  
        processThread.start();
        
        
        SmartDBObject query2 = new SmartDBObject();
        query2.put("$gte", 400286);
        query2.append("$lte", 400315);
