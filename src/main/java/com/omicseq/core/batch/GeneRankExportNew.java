...
public class GeneRankExportNew extends AbstractLifeCycle {
    private static GeneRankExportNew single = new GeneRankExportNew();
    ...
    protected Callable<Object> getCallable() {
        return new GeneRankExportNewCallable(new GeneRankExportNew());
    }
    ...
    static class GeneRankExportNewCallable extends BaseCallable<Object, GeneRankExportNew> {
        public GeneRankExportNewCallable(GeneRankExportNew ref) {
        	super(ref);
        }
...
//                            }
    ...
    public XSSFWorkbook buildWorkbook(GeneRankCriteria criteria) {
        ...
        String[] arr = "Rank,DataSetID,DataType, Cell,Factor,Detail,Order/Total,Percentile(%),value1,value2,Study,Lab".split(",");
		...
		comparatorList.add(ComparatorFactory.getMixtureComparator2(SortType.ASC));
		comparatorList.add(ComparatorFactory.getTssTesCountComparator2());
		Collections.sort(ranks, new ComparatorChain(comparatorList));
        ...
        CellUtil.createCell(header2, 0, "value1 Remark");
        CellUtil.createCell(header2, 1, "value2 Remark");
        
        XSSFRow row2 = st2.createRow(1);
        CellUtil.createCell(row2, 0, "statistic Read of this dataset in GeneBody,get the sample_readcount,because every GeneBody's length is not same,value1=sample_readcount/len(Genebody)。");
        CellUtil.createCell(row2, 1, "statistic Read of this dataset in Promoter，get the sample_readcount，it will be normalized and then value1=sample_readcount/10000");
        
        XSSFRow row3 = st2.createRow(2);
        CellUtil.createCell(row3, 0, "If this dataset contains INPUT, deal value1\n first to get sample and input value1，then exclude value1 of INPUT as :sample_readcount-(input_readcount*inputtotalread/sampletotalread)");
        ...
        CellUtil.createCell(row5, 0, "TCGA Assemble Of RNA-seq ExperimentType value1 uesed experimental_protocol Of RPKM\nICGC Assemble of RNA-seq ExperimentType value1 uesed experimental_protocol Of RSEM");
    ...
    public void expXlsx(Integer geneId) {
...
//    	{
//        	GeneRankExportNew  ex = new GeneRankExportNew();
//        	ex.expXlsx(geneIds.get(i));
//    	}
    }

    public static GeneRankExportNew getInstance() {
        return single;
    }
