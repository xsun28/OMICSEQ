...
public class CalculateGeneRankOfCNVSummary {
...
//		String[] cancerTypes = {"PRAD", "ACC", "BLCA", "BRCA", "CESC", "COAD", "DLBC", "ESCA", "GBM", "HNSC", "KICH", "KIRC", "KIRP",
//	        		"LAML", "LGG", "LIHC", "LUAD", "LUSC", "OV", "PAAD", "READ", "SARC", "SKCM", "STAD", "THCA", "UCEC", "UCS"};
		
		String[] cancerTypes = {"LGG", "LIHC", "LUAD", "LUSC", "OV", "PAAD", "READ", "SARC", "SKCM", "STAD", "THCA", "UCEC", "UCS"};
		CalculateGeneRankOfCNVSummary g = new CalculateGeneRankOfCNVSummary();
		for(int i=0 ;i < cancerTypes.length; i++)
		{
	    	...
	    	query.put("source", 1);
	    	query.put("etype", ExperimentType.CVN.getValue());
	    	query.put("deleted", 0);
...
//			g.createGeneRankOfTumorStd(cancerType.toLowerCase());  //create tumor std dev AND total tumor/normal diff
			
//			g.createGeneRanksOfNormal(cancerType.toLowerCase());
...
//		System.out.println("dav:" + dav + "\t sed:" + sed + "\t T:" + t + "\n");
	...
	private void createGeneRanksOfNormal(String cancerType) {
	...
	private void createGeneRankOfTumorStd(String cancerType)
	{
		SmartDBObject query = new SmartDBObject("source", 1);
		query.put("etype", 11);
...
//		{
//			if("".equals(sampleList_normal_average.get(0).getInputSampleIds()) || sampleList_normal_average.get(0).getInputSampleIds().isEmpty() || sampleList_normal_average.get(0).getInputSampleIds().split(",").length <= 1)
//			{
//				return;
//			}
//			
...
//		}
		...
		{
			if("".equals(sampleList_matched_tumor_normal_diff.get(0).getInputSampleIds()) || sampleList_matched_tumor_normal_diff.get(0).getInputSampleIds().isEmpty() || sampleList_matched_tumor_normal_diff.get(0).getInputSampleIds().split(",").length <= 1)
			{
				return;
			}
...
//		{
//			if("".equals(sampleList_normal_std_dev.get(0).getInputSampleIds()) || sampleList_normal_std_dev.get(0).getInputSampleIds().isEmpty() || sampleList_normal_std_dev.get(0).getInputSampleIds().split(",").length <= 1)
//			{
//				return;
//			}
//			
