	...
	<head>
		...
		<script type="text/javascript">
			...
			$(document).ready(function(){
				showTab('variation');
				rootPath = '${ctx}';
				if($("#searchForm_variation").val().trim()!=''){
				...
				<div id="relateinfo" class="span10 text-center">
					<c:if test="${!isHistoryResult}">
						<div class="row" id="topinfo">
							...
							<strong>${variationGene }</strong>"<fmt:message key="label.find"/> ${totalRecords} (<fmt:message key="lable.topone"></fmt:message><strong>${result.total_all}</strong>)
							...
							(${result.usedTime } <fmt:message key="label.seconds"/>) <fmt:message key="label.about"/> 
								 ...
								 <br>(<c:if test="${genome == 'Human'}">hg19</c:if><c:if test="${genome=='Mouse'}">mm9</c:if>): Chr: ${result.current.seqNameShort }, Start: ${result.current.start }
							<!-- <c:if test="${totalRecords gt 0}"> </c:if> -->
						</div> 
						<div class="row infoRow">
							<fmt:message key="label.searchmore1"/>
							<a target="_blank" href="http://www.ncbi.nlm.nih.gov/snp/?term=${variationGene }">NCBI_SNP</a>,
							<a target="_blank" href="http://gvs.gs.washington.edu/GVS141/PopStatsServlet?searchBy=dbsnp+rsID&target=${variationGene }&upstreamSize=0&downstreamSize=0&x=729&y=428">GVS</a>
					...
					<div class="row paginateWrap">
						...
						<c:if test="${page gt 1}"><a href="javascript:;" class="paginatebtn" onclick="prevPage_variation();"><fmt:message key="button.previous"/></a></c:if>
						<c:if test="${totalPage == 0 }">
							<a href="javascript:;" class="inactive">1</a> 
						</c:if>
						<c:forEach var="aPage" begin="${beginPage}"  end="${endPage}">
							<span href="javascript:;" <c:if test="${page eq aPage}">class="current paginate"</c:if> <c:if test="${page != aPage}">class="paginate variation"</c:if>
						...
						<c:if test="${page lt totalPage}"><a href="javascript:;" class="paginatebtn" onclick="nextPage_variation();"><fmt:message key="button.next"/></a></c:if>
					</div>
						...
						<table id="result_detail" class="table table-striped table-bordered table-hover table-condensed">
							...
							<c:forEach items="${result.sampleItemList}" var="item" varStatus="status">
								<tr class="sampleRow">
									...
									<td class="percent" title="${item.variationOrderNoRank }" nowrap="nowrap">${item.variationOrderNoRank}</td>
