	...
	<head>
		...
		<script type="text/javascript">
			...
			$(document).ready(function(){
				...
				if($("#searchForm_multigene").val().trim()!=''){
				...
				<div id="relateinfo" class="span10 text-center">
					<c:if test="${!isHistoryResult}">
						<div class="row" id="topinfo">
							<fmt:message key="button.multigenesearch"/> <fmt:message key="label.find1"/> ${totalRecords} <fmt:message key="label.result"/> 
						...
						<c:if test="${geneItemList[1] != null}"> 
							<div class="row linkRow">
								<fmt:message key="label.wildfind.pathway"/> 
									<c:forEach items="${geneItemList}" var="geneitem" varStatus="status">
										<c:if test="${geneitem.txName != result.current.txName }">
											<span class="linkWord_multigene">${geneitem.txName }<c:if test="${status.count lt geneSize}">,</c:if></span>
										</c:if>
									</c:forEach>
					...
					<div class="row paginateWrap">
						...
						<c:if test="${page gt 1}"><a href="javascript:;" class="paginatebtn" onclick="prevPage_multigene();"><fmt:message key="button.previous"/></a></c:if>
						<c:if test="${totalPage == 0 }">
							<a href="javascript:;" class="inactive">1</a> 
						</c:if>
						<c:forEach var="aPage" begin="${beginPage}"  end="${endPage}">
							<span href="javascript:;" <c:if test="${page eq aPage}">class="current paginate"</c:if> <c:if test="${page != aPage}">class="paginate multigene"</c:if>
						...
						<c:if test="${page lt totalPage}"><a href="javascript:;" class="paginatebtn" onclick="nextPage_multigene();"><fmt:message key="button.next"/></a></c:if>
					</div>
					<div class="row">
						...
						<table id="result_detail" class="table table-striped table-bordered table-hover table-condensed">
							<tr>
								<th width="8%">
									...
									<c:if test="${!isHistoryResult}">
										<a style="cursor:pointer;" onclick="list_multigene_BySort(this);">
							...
							<c:forEach items="${result.sampleItemList}" var="item" varStatus="status">
								<tr class="sampleRow">
									...
									<td class="percent" title="${item.pathwayOfRank*100 }" nowrap="nowrap">${item.pathwayOfRank*100}</td>
